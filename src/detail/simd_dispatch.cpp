...
// Forward declarations — x86 SSE2/AVX2/AVX-512 (only linked on x86)
...
bool validate_utf8_dispatch(const char* data, size_t size) {
    ...
    if (f.neon)    return neon::validate_utf8(data, size);
#endif
...
#elif defined(__aarch64__) || defined(_M_ARM64)
    if (f.neon)    return neon::count_code_points(data, size);
#endif
    // Scalar fallback
    size_t count = 0;
